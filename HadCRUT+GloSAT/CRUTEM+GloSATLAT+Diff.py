...
# Comparison plot: CRUTEM, GloSATLAT, and difference
...
from CRUTEM.load import load_month as load_month_crutem, get_land_mask
from GloSAT.GloSATLAT.load import load_month as load_month_glosatlatlat
...
for year in range(start.year, end.year + 1):
    ...
    for month in range(1, 13):
        ...
        hdata = load_month_crutem(year, month, new_grid=new_grid)
        hdata.data.data[hdata.data.mask] = np.nan  # Mask the data
        gdata = load_month_glosatlatlat(year, month, new_grid=new_grid)
        ...
        if ndata_crutem is None:
            ndata_crutem = ndmoh
        else:
            ndata_crutem = np.concatenate((ndata_crutem.data, ndmoh.data), axis=1)
            ndata_crutem = np.ma.MaskedArray(
                ndata_crutem.data, np.isnan(ndata_crutem.data)
            )
        if ndata_glosatlat is None:
            ndata_glosatlat = ndmog
        else:
            ndata_glosatlat = np.concatenate((ndata_glosatlat.data, ndmog.data), axis=1)
            ndata_glosatlat = np.ma.MaskedArray(
                ndata_glosatlat.data, np.isnan(ndata_glosatlat.data)
            )

ndata_difference = ndata_crutem - ndata_glosatlat

# # Filter
if args.convolve != "none":
    ndata_crutem = csmooth(args.convolve, ndata_crutem)
    ndata_glosatlat = csmooth(args.convolve, ndata_glosatlat)
...
crutem_levels = np.quantile(
    np.concatenate(
        (
            ndata_crutem.compressed(),
            ndata_glosatlat.compressed(),
        )
    ),
    np.linspace(0, 1, cmap.N + 1),
    method="linear",
)
crutem_norm = colors.BoundaryNorm(crutem_levels, cmap.N)
...
ax_glosatlat = fig.add_axes([0, (0.05 + 1) / 3, 1, 0.95 / 3])
plot_dataset(
    ax_glosatlat,
    dts,
    ndata_glosatlat,
...
ax_crutem = fig.add_axes([0, (0.05 + 2) / 3, 1, 0.95 / 3])
plot_dataset(
    ax_crutem,
    dts,
    ndata_crutem,
...
fig.savefig(
    "%s/%s_%s_%s.png" % (".", "CRUTEM+GloSATLAT+Diff", args.reduce, args.convolve)
)
