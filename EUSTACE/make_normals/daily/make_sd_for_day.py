...
# Make the 1961-1990 standard deviations from EUSTACE1.0 data 
#  for a given day.
...
op_dir = "%s/sd_%04d_%04d" % (root_dir,args.startyear,args.endyear)
...
for cyr in range (args.startyear,args.endyear+1):
  ...
  if rms is None:
      ...
      count.data.mask = False
      count.data *= 0
      count.data[rms.data.mask==False] += 1
  else:
      rms.data.mask[inst.data.mask==False] = False
      rms.data[rms.data<0] *= 0
      rms.data = rms.data + (inst.data-accum.data)**2
      count.data[inst.data.mask==False] += 1

rms.data[count.data>=20] /= count.data[count.data>=20]
rms.data=numpy.sqrt(rms.data)
rms.data.mask[count.data<20] = True

iris.save(rms,"%s/%02d%02d.nc" % (op_dir,args.month,args.day),
          fill_value=-32768)
