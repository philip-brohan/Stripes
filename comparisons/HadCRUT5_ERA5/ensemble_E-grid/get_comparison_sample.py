...
# Get the sample cube for HadCRUT5-ERA5 comparison
...
def get_sample_cube(start=datetime.datetime(1851,1,1,0,0),
    ...
    slice_dir = "%s/EUSTACE/derived/HadCRUT5_ERA5_comparison" % \
                                    os.getenv('SCRATCH')

    # Array to store the sample in
    ndata=numpy.ma.array(numpy.zeros(((2019-1979)*12,720)),mask=False)
    dts=[]
